# Caso Piloto — Bodega Mayran (Rehabilitación de Cubierta Post-Incendio)
...
Aclaración de modalidad: a diferencia de los 4 casos de la Ronda 1 (2026-07-16, primera tanda), este proyecto está **100% concluido y entregado** (Acta de Entrega-Recepción firmada 17-Abr-2026, avance físico final 100%). Esto lo distingue de Chevrolet Pedregal (que seguía en etapa de cotización) y permite reconstruir el ciclo completo: levantamiento → cotización → discrepancia interna de precios → ejecución → incidente en obra → cobranza → cierre. Es el caso con el ciclo de vida más completo disponible en el repositorio de PICC a la fecha de esta reconstrucción.

## Fuentes de datos usadas (única fuente de verdad para este caso)

- `C:\Development\DataManager\proyectos\MAYRAN-2026-001\proyecto.json`
- `C:\Development\DataManager\proyectos\MAYRAN-2026-001\HISTORIAL.md`
- `C:\Development\DataManager\proyectos\MAYRAN-2026-001\10_REPORTES\ACTA_ENTREGA_RECEPCION_MAYRAN_20260417.md`
- `C:\Development\DataManager\proyectos\MAYRAN-2026-001\10_REPORTES\REPORTE_AVANCE_03_20260318.md`
- `C:\Development\DataManager\proyectos\MAYRAN-2026-001\09_FINANZAS\ESTADO_CUENTA.md`
- `C:\Development\DataManager\proyectos\MAYRAN-2026-001\archivo_historico\ANALISIS_DIFERENCIAS.md`
- `C:\Development\DataManager\proyectos\CARDEAL-2026-001\QARDEAL Andrade.pdf` (presupuesto detallado por partidas, 21-Feb-2026 — ver Sección 4, discrepancia de fuente)
- `C:\Development\DataManager\proyectos\MAYRAN-2026-001\07_FOTOS_AVANCE\` (nombres de archivo con fecha y descripción de las 5 subcarpetas: `01_ESTRUCTURA`, `02_INSTALACIONES`, `03_ACABADOS`, `04_GENERAL`, `05_INCIDENTE_AICM`) — solo se usaron nombres de archivo con fecha y descripción textual, nunca se interpretó contenido visual no descrito en el nombre o en un documento asociado

**Nota de trazabilidad crítica — hallazgo de esta reconstrucción:** el proyecto **NISSAN-ANDRADE-2026-001** (ver `caso_nissan_andrade.md`) y este proyecto comparten el mismo cliente (**Grupo Andrade / Partarrio**, contacto Rafael Chapa H.). Además, el Acta de Entrega-Recepción de este proyecto (Sección II) identifica al cliente formal como **DENG MOTORS, SA DE CV, marca comercial "QARDEAL"** (alquiler de automóviles sin chofer, RFC DMO2410305D1, con domicilio fiscal en Cuernavaca, Morelos). Esto **NO es el mismo proyecto** que "AEROPUERTO QarDeal", declarado **CANCELADO** en la memoria del sistema ZEUS (corrección Pablo, 22-Jun-2026). Son dos entidades homónimas distintas:
1. **QARDEAL (marca comercial de DENG MOTORS, SA DE CV)** — cliente de Grupo Andrade, renta de autos sin chofer, es el cliente que firma el Acta de este proyecto (Bodega Mayran).
2. **AEROPUERTO QarDeal** — un proyecto de infraestructura distinto y ya cancelado, mencionado en `NISSAN-ANDRADE-2026-001/04_ENTREGABLES/PROPUESTA_PRELIMINAR_NISSAN_20260318.md` como antecedente de relación comercial con el mismo Grupo Andrade ("Rafael Chapa y Partarrio conocen la capacidad de PICC a través del proyecto AEROPUERTO QarDeal").

Se documenta esta distinción explícitamente porque la carpeta `CARDEAL-2026-001` (nombre parecido a "QarDeal" por error de captura/homofonía) contiene en realidad una **cotización temprana de este mismo proyecto Bodega Mayran** (mismo cliente, misma dirección "Laguna de Mayran 48", mismo código interno alterno "ANDRADE-2026-001", fechada 21-Feb-2026), no un proyecto de oficinas ni el proyecto cancelado del aeropuerto. Ver Sección 4 para el detalle de la discrepancia de cifras entre esa cotización temprana y el contrato final.
...
| Campo | Valor |
| --- | --- |
| Nombre del proyecto | Bodega Mayran — Rehabilitación Cubierta Post-Incendio (MAYRAN-2026-001) |
...
| Retiro de asbesto y manejo de residuo peligroso como partida crítica de cumplimiento regulatorio | E (regulatorio) | S3 — Riesgo alto (manejo de residuo peligroso; requiere manifiesto certificado) | E3 — Evidencia operativa (partida explícita en presupuesto de `CARDEAL-2026-001`, "Retiro de vegetacion invasiva" y limpieza confirmadas; manifiesto de residuos en `ANALISIS_DIFERENCIAS.md` Fase 1) | 5 | Soportada |
...
3. Reconciliar las dos cotizaciones (`CARDEAL-2026-001` vs. el monto de `HISTORIAL.md`) antes de que esta discrepancia se repita en otro proyecto del mismo cliente — mismo patrón que DEF-03 de la Ronda 1 (Chevrolet Pedregal).
